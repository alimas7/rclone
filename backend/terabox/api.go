...
retry:
	...
	if retry == 0 && opts.Method == http.MethodPost && opts.MultipartParams != nil {
		var overhead int64
		var err error
		opts.Body, opts.ContentType, overhead, err = rest.MultipartUpload(ctx, opts.Body, opts.MultipartParams, opts.MultipartContentName, opts.MultipartFileName, opts.ContentType)
		if err != nil {
			return err
		}
